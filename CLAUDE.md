...
- `user-manual-install-apps.sh` — orchestrator for the curated application set (`docs/specs/2026-06-03-reproduce-manual-apps-design.md`). Runs the seven app installers below in order (docker → chrome → mullvad → flatpaks → node/npm-globals → uv → claude), gating each, passing through --dry-run / --no-prompt. Runs as the user; children self-elevate. NVIDIA excluded. Mirrors `bootstrap-fedora.sh`.
