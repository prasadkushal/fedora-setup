...
- `user-manual-configure-ssh-server.sh` — enable + start `sshd` and ensure the firewalld `ssh` service (`docs/specs/2026-06-07-remote-access-design.md`). sshd_config stays at Fedora defaults. Auto-sudo, --dry-run / --no-prompt.
...
- `docs/specs/2026-06-07-remote-access-design.md` — SSH (native sshd) + RDP
  (KDE krdp, not xrdp) remote access; why system-user auth and per-user
  privilege model.
